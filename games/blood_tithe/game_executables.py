"""Blood Tithe game logic — Blood Gauge fill, H multiplier, Blood Moon retrigger."""
...
class GameExecutables(GameCalculations):
    ...
    def get_clusters_update_wins(self):
        ...
        Wild symbols expand L and H clusters via custom BFS (SDK clustering disabled for wilds).
        """
        # SDK clustering with wild disabled — our BFS in evaluate_clusters_with_gauge handles wilds
        clusters = Cluster.get_clusters(self.board, wild_key="_disabled_")
        return_data = {"totalWin": 0, "wins": []}

        # Two-pass evaluation: L first → gauge fill → persist check → H with new mult
        self.board, self.win_data, l_clusters, h_clusters, l_sym_count, gauge_delta, wild_info = (
        ...
        self._last_wild_info = wild_info
